...
if (!clientId || !clientSecret) {
  console.error('❌ FEIL: SPOTIFY_CLIENT_ID og SPOTIFY_CLIENT_SECRET må være satt i din .env-fil.');
...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === '/callback') {
    ...
    if (error) {
      ...
      res.end(`<h1>Kunne ikke autorisere</h1><p>Spotify returnerte en feil: ${error}</p>`);
    ...
    res.end('<h1>Vellykket!</h1><p>Du kan lukke dette vinduet nå. Sjekk terminalen din for Refresh Token.</p>');
    ...
    try {
      ...
      console.error('❌ Feil under veksling av kode:', err.message || err);
      cleanupAndExit(1);
    }
  } else {
...
server.listen(PORT, () => {
  ...
  exec(`${openCmd} "${authUrl}"`, (err) => {
    if (err) {
      console.log('(Klarte ikke å åpne nettleseren automatisk, vennligst kopier lenken over.)');
    }
  });
});
