...
let mockPlaylistTracks: SpotifyTrack[] = [
  ...
  },
  {
    ...
    albumName: "Version",
    albumImageUrl:
      "https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4?auto=format&fit=crop&w=150&h=150&q=80",
...
const mockCatalog: SpotifyTrack[] = [
  ...
  },
  {
    ...
    albumName: "Whitney",
    albumImageUrl:
      "https://images.unsplash.com/photo-1498038432885-c6f3f1b912ee?auto=format&fit=crop&w=150&h=150&q=80",
  ...
  },
  {
    ...
    albumImageUrl:
      "https://images.unsplash.com/photo-1465847899084-d164df4dedc6?auto=format&fit=crop&w=150&h=150&q=80",
  ...
  },
  {
    ...
    albumName: "Thriller",
    albumImageUrl:
      "https://images.unsplash.com/photo-1487180142328-0c4e37023af5?auto=format&fit=crop&w=150&h=150&q=80",
  ...
  },
  {
    ...
    albumImageUrl:
      "https://images.unsplash.com/photo-1506157786151-b8491531f063?auto=format&fit=crop&w=150&h=150&q=80",
...
export async function searchTracks(
  ...
  try {
    ...
    return (data.tracks?.items || []).map((item: SpotifyTrackItem) => ({
      id: item.id,
      name: item.name,
      artists: item.artists.map((a: SpotifyArtist) => a.name).join(", "),
...
export async function getPlaylistTracks(env?: Env): Promise<SpotifyTrack[]> {
  ...
  try {
    ...
    const tracks: SpotifyTrack[] = (data.items || [])
      .map((item: SpotifyPlaylistItem) => item.track || item.item) // Support both old 'track' and new 'item' shapes (from Feb 2026 API changes)
      .filter(
        (t: SpotifyTrackItem | undefined): t is SpotifyTrackItem =>
          !!t && (t.type === "track" || !!t.name),
      ) // Filter out null/deleted tracks/episodes
      .map((t: SpotifyTrackItem) => {
        return {
          id: t.id || "",
          name: t.name || "",
          artists: (t.artists || [])
