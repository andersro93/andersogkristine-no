 ...
 */
export function getEnvVar<K extends keyof Env & string>(
  key: K,
  localEnv?: Env,
): Env[K] {
  return (
 ...
 */
export function requireEnvVar(key: keyof Env & string, localEnv?: Env): string {
  const value = getEnvVar(key, localEnv);
  if (!value || typeof value !== "string") {
    throw new Error(
      `${String(key)} is not defined or is not a string. Please check your environment configuration or .env file.`,
    );
  }
  return value;
}
