  ...
  useEffect(() => {
    async function loadPlaylist() {
      try {
        ...
        console.error(err);
        showToast('Klarte ikke å laste spillelisten. Prøv å laste siden på nytt.', 'error');
      } finally {
    ...
    try {
      ...
      if (data.success) {
        ...
        setSearchResults((prev) =>
          prev.map((t) => (t.uri === track.uri ? { ...t, alreadyAdded: true } : t))
        );
      ...
      console.error(err);
      showToast(err.message || 'Klarte ikke å legge til sangen. Vennligst prøv igjen.', 'error');
    } finally {
      setAddingUri(null);
    }
  }
    ...
    <div className="space-y-12 font-sans text-brand-title select-none">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-6 bg-brand-bg/40 border border-brand-title/10 rounded-2xl p-6 md:p-8">
        <div className="text-center sm:text-left space-y-2">
          <h3 className="font-serif text-2xl text-brand-title font-medium">Bli med på å lage spillelisten!</h3>
          <p className="text-sm text-brand-text/75 max-w-md leading-relaxed">
            Her kan du foreslå låter du vil høre på dansegulvet. Klikk under for å åpne og følge listen direkte på din Spotify.
          </p>
        </div>
      ...
      <div className="space-y-6">
        <div className="max-w-xl mx-auto space-y-2">
          <label htmlFor="music-search" className="block text-center font-serif text-xl text-brand-title font-medium">
          ...
          <div className="relative">
            ...
            {isSearching && (
              <div className="absolute right-4 top-1/2 -translate-y-1/2">
                <svg className="animate-spin h-5 w-5 text-brand-title/50" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
      ...
      <div className="space-y-6">
        <div className="text-center">
          <h2 className="font-serif text-3xl text-brand-title">Ønskede låter</h2>
          <p className="text-xs text-brand-text/70 uppercase tracking-widest mt-1">
            {isLoadingPlaylist ? 'Laster spilleliste...' : `${playlistTracks.length} sanger foreslått`}
          </p>
        </div>
        ...
        {isLoadingPlaylist ? (
          ...
          <div className="space-y-3 max-w-4xl mx-auto">
            {[1, 2, 3].map((n) => (
              <div key={n} className="flex items-center justify-between p-4 bg-brand-bg/20 rounded-xl border border-brand-title/5 animate-pulse">
          ...
          <div className="text-center py-12 bg-brand-bg/10 rounded-2xl border border-dashed border-brand-title/20 max-w-4xl mx-auto">
            ...
            <p className="font-serif italic text-lg text-brand-title/80 mt-3">Ingen sanger er foreslått enda.</p>
            <p className="text-xs text-brand-text/50 mt-1">Vær den første til å foreslå en sang ved å søke over!</p>
          </div>
        ) : (
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-brand-title/5">
                  {playlistTracks.map((track, idx) => (
                    <tr key={`${track.id}-${idx}`} className="hover:bg-brand-title/1 transition duration-150 text-sm">
                      {/* Cover Number */}
