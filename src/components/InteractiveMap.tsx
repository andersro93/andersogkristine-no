...
export default function InteractiveMap() {
  ...
  useEffect(() => {
    fetch('/api/locations')
      ...
      .catch((err) => {
        console.error(err);
        setError('Det oppstod en feil ved lasting av kartet. Vennligst prøv igjen.');
        setIsLoading(false);
      });
  }, []);
  ...
  useEffect(() => {
    if (isLoading || error || !mapContainerRef.current || mapRef.current) return;

    const L = (window as any).L;
    if (!L) {
      setError('Leaflet.js ble ikke lastet inn. Vennligst prøv å oppdatere siden.');
      return;
    }
    ...
    const mapPane = mapPaneRef();
    if (mapPane) {
      mapPane.style.filter = 'sepia(0.2) contrast(0.95) saturate(0.9) brightness(1.02)';
    }
  ...
  const mapPaneRef = () => {
    ...
    return mapContainerRef.current.querySelector('.leaflet-tile-pane') as HTMLDivElement;
  };
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        const marker = L.marker([latitude, longitude], { icon: userIcon })
          ...
          .bindPopup('<div class="font-sans text-xs font-semibold p-1">Du er her</div>');
      ...
      (err) => {
        console.error(err);
        alert('Klarte ikke å hente posisjonen din. Vennligst sjekk stedstjenester.');
        setUserLocationActive(false);
      },
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[450px] space-y-4">
        ...
        <p className="font-serif italic text-brand-text/70">Laster inn kartet...</p>
      </div>
    );
  }
          ...
          {filteredLocations.length > 0 ? (
            filteredLocations.map((loc) => {
              ...
              return (
                ...
                >
                  <span className="text-xl shrink-0 group-hover:scale-110 transition-transform">{iconSymbol}</span>
                  <div className="min-w-0">
                    <p className="font-serif font-semibold text-brand-title text-base group-hover:text-brand-text transition-colors truncate">{loc.name}</p>
                    <p className="text-[10px] text-brand-text/50 font-sans uppercase tracking-wider font-bold capitalize pt-0.5">{loc.ikon === 'default' ? 'Lokasjon' : loc.ikon}</p>
                  </div>
                </button>
              );
            })
          ) : (
            <p className="text-center font-sans text-sm text-brand-text/60 py-8">Ingen steder matcher søket.</p>
          )}
        </div>
      </div>

      {/* Map Element */}
      <div ref={mapContainerRef} className="flex-1 h-[60%] lg:h-full w-full z-10" />
    </div>
  );
}
