...
export default function RSVPForm({ invite }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setErrorMsg(err.message ?? 'Klarte ikke å sende svar. Vennligst prøv igjen.');
  ...
  if (submitStatus === 'success') {
    return (
      <div className="text-center py-10 space-y-6 animate-fade-in">
        ...
        <h3 className="font-wedding text-4xl text-brand-title">Tusen takk for svar!</h3>
    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8 font-sans">
            ...
            return (
              ...
              >
                ...
                <div className="space-y-2">
                  <label className="block text-xs font-semibold uppercase tracking-wider text-brand-title opacity-95">
                    Kommer du?
                  </label>
                    ...
                    {(['Kommer', 'Kommer ikke'] as const).map((option) => (
                      ...
                      >
                        ...
                        <span className="text-sm font-medium text-brand-title">
                          {option === 'Kommer' ? 'Ja, jeg gleder meg!' : 'Nei, jeg kan dessverre ikke'}
                        </span>
                      </label>
                    ))}
                  </div>
                ...
                <div
                  className={`space-y-2 transition-all duration-300 overflow-hidden ${
                    isAttending ? 'max-h-40 opacity-100' : 'max-h-0 opacity-0 pointer-events-none'
                  }`}
                >
                  <label
        ...
        <div className="text-center pt-4">
          ...
          >
            <span>{submitStatus === 'loading' ? 'Sender...' : 'Send svar'}</span>
